...
module ISet = Set.Make({ type t = int; let compare = compare; });

let koans _ => Mocha.describe "Sets" @@ fun _ => {
  ...
  Mocha.it "Set.Make can take any module that obeys that interface" @@
    fun _ => {
      module IntSet = Set.Make({ type t = int; let compare = compare });
      Mocha.ok true;
    };

  Mocha.it "can be created empty" @@
    fun _ => {
      (__())
        |> ISet.equal ISet.empty
        |> Mocha.ok;
    };
  ...
  Mocha.it "can be created from a list" @@
    fun _ => {
      (ISet.of_list (__()))
        |> ISet.equal (ISet.add 1 ISet.empty)
        |> Mocha.ok;
    };
  ...
  Mocha.it "will not add duplicate elements" @@
    fun _ => {
      (ISet.of_list (__()))
        |> ISet.equal (ISet.of_list [1, 1, 2, 3, 4, 4, 5, 1, 2])
        |> Mocha.ok;
    };
