  ...
  Mocha.describe "Mapionaries" @@ (
    fun _ => {
      ...
      Mocha.it "a key-value pair can be added to the dictionary" @@ (
        fun _ => Some (__ ()) |> Mocha.eq (IMap.get 2 @@ IMap.add 2 "b" @@ IMap.singleton 1 "a")
      );
      Mocha.it "inserting can also overwrite the value associated with a key" @@ (
        fun _ => Some (__ ()) |> Mocha.eq (IMap.get 1 @@ IMap.add 1 "b" @@ IMap.singleton 1 "a")
      );
      ...
      Mocha.it "of course there is a more direct way to remove a value too" @@ (
        fun _ => Some (__ ()) |> Mocha.eq (IMap.get 1 @@ IMap.remove 1 @@ IMap.singleton 1 "a")
      );
      Mocha.it "you can get the list of keys" @@ (
        fun _ => __ () |> Mocha.eq (IMap.keysList @@ IMap.singleton 1 "a")
      );
      Mocha.it "or the list of values" @@ (
        fun _ => __ () |> Mocha.eq (IMap.valuesList @@ IMap.singleton 1 "a")
      );
