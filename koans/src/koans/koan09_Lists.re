...
let koans _ =>
  Mocha.describe "Lists" @@ (
    fun _ => {
      Mocha.it "list literals are denoted by brackets" @@ (fun _ => __ () |> Mocha.eq [1, 2, 3]);
      Mocha.it "length gives the length of a list" @@ (
        fun _ => __ () |> Mocha.eq (List.length [1, 2, 3])
      );
