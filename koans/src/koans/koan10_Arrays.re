  ...
  Mocha.describe "Arrays" @@ (
    fun _ => {
      ...
      Mocha.it "length gives the number of elements" @@ (
        fun _ => __ () |> Mocha.eq (Array.length @@ Array.of_list [0, 1, 2])
      );
      ...
      Mocha.it "it is easy to create an empty array" @@ (
        fun _ => Array.of_list (__ ()) |> Mocha.eq Array.empty
      );
      Mocha.it "or to create an array of all the same value" @@ (
        fun _ => Array.of_list (__ ()) |> Mocha.eq (Array.make 3 "a")
      );
      ...
      Mocha.it "mapping is just like with lists" @@ (
        fun _ =>
          Array.of_list (__ ()) |> Mocha.eq (Array.map (fun i => i * 3) @@ Array.of_list [0, 1, 2])
      );
      Mocha.it "you can get the value at an index" @@ (
        fun _ => Some (__ ()) |> Mocha.eq (Array.get_safe (Array.of_list [0, 1, 2]) 1)
      );
      ...
      Mocha.it "or get a slice of the array" @@ (
        fun _ => Array.of_list (__ ()) |> Mocha.eq (Array.sub (Array.of_list [0, 1, 2, 3, 4]) 1 4)
      )
    }
  );
